/*
 * File Synopsis:
 * This file contains the methods, allocators, and deallocators needed to create a priority queue.
 * There are 7 functions and helper functions along with 2 testing functions. There are a variety of
 * added tests at the bottom of this file to ensure that the priority queue works as intended for a
 * wide variety of inputs. There is more information on the priority queue in the header file.
 */
...
/*
 * Function Synopsis:
 * The enqueue function takes in one parameter which is a Datapoint to be added to the array.
 * This function has a void return type, so no value is returned.
 */
void PQArray::enqueue(DataPoint elem) {
    ...
    for(int i = _numFilled-1; i>=0; i--){
        if(_elements[i].priority < elem.priority){
            swap(i,i+1);
        }
        else{
            break;
...
/* Function synopsis:
 * Added by student, this helper function is called to double the size of allocated spaces in the array.
 * It is a void function so nothing is returned and there are no parameters.
 */
void PQArray::enlargeSize(){
...
STUDENT_TEST("PQArray stress test, cycle many random elements in and out") {
    ...
    setRandomSeed(57);//make test behavior deterministic

    DataPoint mostUrgent = { "", 0 };
    pq.enqueue(mostUrgent);
    while (true) {
        if (++n < maxEnqueues && randomChance(0.7)){
...
STUDENT_TEST("PQArray enqueues then dequeues all"){
    PQArray pq;
...
STUDENT_TEST("Timing data 1/5 for enqueue"){
    PQArray pq;
    int n = 5000;
...
STUDENT_TEST("Timing data 2/5 for enqueue"){
    PQArray pq;
    int n = 10000;
...
STUDENT_TEST("Timing data 3/5 for enqueue"){
    PQArray pq;
    int n = 20000;
...
STUDENT_TEST("Timing data 4/5 for enqueue"){
    PQArray pq;
    int n = 40000;
...
STUDENT_TEST("Timing data 5/5 for enqueue"){
    PQArray pq;
    int n = 80000;
